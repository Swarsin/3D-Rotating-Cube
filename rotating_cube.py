...
#     dot_product = sum([sum(x * y for x, y in zip(rows_x, cols_y)) for rows_x, cols_y in zip(x, zip(*y))])

#     if cols_x == rows_y:
#         for i in range(rows_x):
#             for j in range(cols_y):
#                 for k in range(rows_y):
#                     dot_product[i][j] += x[i][k] * y[k][j]
#     else:
#         print("Can't do matrix multiplication - incompatible sizes")
#     return dot_product

# while True:
#     clock.tick(60)
#     for point in cube_points:
#         point_2d = multiply_matrices()
